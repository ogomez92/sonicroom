...
On top of the single primary voice mic, a user can stream **additional input devices at once**, each as its **own separate producer** (`source: "mic"`) — built exactly like `share`/`file`: a separate track that **forces the room onto the SFU** (`room.extraMicStreamers`, mirroring `sharers`/`fileStreamers`) and is auto-tapped by recording/streaming. The picker is an accessible checkbox list in `DeviceSettings` (every input device **except** the primary mic and the empty "Default" id, so the same capture can't stream twice), with a **per-device mono/stereo** radio. Persisted as `sonicroom:streamedMicDeviceIds` + `sonicroom:micStereoByDevice` (`streamedMicDeviceIds` / `micStereoByDevice` in the store), carried lobby→call.

- **Client** (`useMediasoup.ts`): `extraMicsRef` (keyed by deviceId) owns each device's `getUserMedia` stream → its own destination → producer. A **reconcile effect** diffs the store selection against the live producers — **in-call only** (gated on `connected`; the lobby just tracks the baseline, which is applied at join). It starts added devices, stops removed ones, and **restarts just the one** whose mono↔stereo flipped (the codec can't renegotiate live; in a ≤2-peer room that one device flips SFU↔P2P briefly — the same machinery as the 2↔3 boundary — but a 3+ room stays SFU regardless). `produceAllExtraMics` re-produces every selected device in `setupSfuInner` on a mode switch / reconnect; the `extraMic` join flag re-pins SFU first.
